...
RUN cd ~/; mkdir -p /code/node; wget https://nodejs.org/dist/v4.1.0/node-v4.1.0-linux-x64.tar.gz; tar -zxvf node-v4.1.0-linux-x64.tar.gz; mv node-v4.1.0-linux-x64.tar.gz node; cd node; yum install gcc gcc-c++; ./configure; make; make install; node --version; cd /code/node; echo 'var http = require("http");\n\n' > server.js; echo 'http.createServer(function(request, response) {\n' >> server.js; echo 'response.writeHead(200, {"Content-Type": "text/plain"});\n' >> server.js; echo 'response.write("Hello World");\n' >> server.js; echo 'response.end();\n' >> server.js; echo '}).listen(8080);' >> server.js; 
